...
// Find and fix the handleSalesAndBill block one last time to be absolute sure
const startTag = "const handleSalesAndBill = async (saleData, originalSale, loyaltyUpdate) => {";
const endTag = "const handleSaleSubmit = async (e) => {";
...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    const cleanBlock = `const handleSalesAndBill = async (saleData, originalSale, loyaltyUpdate) => {
...
`;
    ...
    fs.writeFileSync(filePath, before + cleanBlock + after);
    console.log('✅ Re-written the file section cleanly.');
} else {
